...
| ------------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OTEL_EXPORTER_OTLP_ENDPOINT`         | yes\*    | OTLP exporter base URL. The signal path (`/v1/traces`, `/v1/metrics`) is appended automatically, matching the OTLP spec. Set the base URL (e.g. `https://otlp-gateway-prod-ap-northeast-0.grafana.net/otlp`) — do not include the signal path yourself, or the collector will return 404. |
...
| `OTEL_EXPORTER_OTLP_METRICS_ENDPOINT` | no       | Metrics-specific override used verbatim, same precedence rule as `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`. If neither this nor `OTEL_EXPORTER_OTLP_ENDPOINT` resolves to a metrics endpoint, metrics are not exported and `@opentelemetry/api`'s no-op MeterProvider stays in effect.         |
...
| `OTEL_EXPORTER_OTLP_METRICS_HEADERS`  | no       | Metrics-specific header override, same precedence rule as `OTEL_EXPORTER_OTLP_TRACES_HEADERS`.                                                                                                                                                                                            |
